...
export const LOGIC_GYM_ACTIVITIES: LogicGymActivityDefinition[] = [
  ...
  { key: "mathDash", title: "Corsa Numerica", glyph: ">_", icon: "gym-math-dash", theme: "Azione matematica", color: 0xff6d8d, desc: "Sposta il drone tra le corsie e intercetta solo il risultato corretto prima che scappi via." },
  { key: "multipleDefense", title: "Difesa dei Multipli", glyph: "!", icon: "gym-multiple-defense", theme: "Azione e divisibilita", color: 0x9f8cff, desc: "Proteggi il nucleo: colpisci al volo i numeri che rispettano la regola, evita le esche." },
...
export function logicGymActivityLevelLine(key: GymActivityKey, level: number, metrics: LogicGymLevelMetrics): string {
  switch (key) {
    case "mathDash": return `${metrics.actionWaveCount} varchi - ${level >= 5 ? "trappole veloci" : "3 corsie guidate"} - frecce o touch`;
    case "multipleDefense": return `${metrics.actionWaveCount} ondate - ${level >= 5 ? "regole miste" : "multipli e pari"} - clicca i bersagli`;

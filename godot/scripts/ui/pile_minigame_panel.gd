...
func _costruisci() -> void:
	...
	consegna.custom_minimum_size = Vector2(PER_FILA * LATO + 40.0, 0)
	...
	_griglia.custom_minimum_size = Vector2(PER_FILA * LATO, float(file) * LATO)
...
func _crea_pezzo(indice: int) -> Control:
	...
	pezzo.custom_minimum_size = Vector2(LATO - 4.0, LATO - 4.0)
	pezzo.position = Vector2(
		float(indice % PER_FILA) * LATO,
		float(indice / PER_FILA) * LATO)
	pezzo.focus_mode = Control.FOCUS_NONE

...
func _capture_profile(
	...
	var mobile_budget := int(profile.get("performanceBudget", {}).get("mobile", {}).get("maxDrawCalls", 700))
	var strict_budget := mini(web_budget, mobile_budget)
	if draw_calls > 0 and draw_calls > strict_budget:
		push_error("WORLD PROFILE RENDER probe: %d draw call oltre budget tablet/mobile %d nel mondo %d" % [
			draw_calls, strict_budget, level])
		return ERR_OUT_OF_MEMORY
	print("WORLD PROFILE RENDER — mondo %d %s: %d draw call (budget mobile %d, Web %d)" % [
		level, suffix if suffix != "" else "HUD", draw_calls, mobile_budget, web_budget])
	...
	for level in [21, 22, 23]:
		...
		if level in [21, 22, 23]:
			if await _capture_profile(level, Vector2i(1440, 900), "-landmark-clean", false, true) != OK:
				...
				quit(2)
				return
	print("WORLD PROFILE RENDER probe OK — Gate E1 mondi 21/23, HUD + clean desktop/compatto + landmark")
	quit(0)

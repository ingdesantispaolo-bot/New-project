  ...
  completeProceduralMission(): void {
    ...
    if (proceduralRunRules.modeFor(run) === "training") {
      ...
      saveSystem.addJournalEntry({
        ...
        lines: [
          `Settore concluso: ${proceduralScoring.domainLabel(result.focus)}. Hai completato ${solvedCount}/${requiredCount} prove specialistiche.`,
          `Profondità: ${run.difficulty}. Seed: ${run.seed}`,
          `Tempo: ${formatDuration(result.elapsedMs)}. Miglior tempo su questo settore/profondità: ${formatDuration(record.bestTimeMs)}.`,
          ...
          "Questo percorso è calibrazione: puoi ripeterlo per abbassare il tempo, ma il voto resta legato a precisione, profondità e uso degli aiuti.",
        ],
    ...
    saveSystem.addJournalEntry({
      ...
      lines: [
        ...
        `Seed: ${run.seed}`,
        `Settore: ${proceduralScoring.domainLabel(focus)}. Profondità completata: ${run.difficulty}. Prossima run consigliata: ${nextDifficulty}.`,
        `Prove risolte: ${solvedCount}/${requiredCount}. Indizi usati: ${run.hintsUsed}. Tempo: ${formatDuration(elapsed)}.`,
        `Punteggio formativo: ${run.score?.total ?? 0}. Bonus assegnati per profondità, velocità ragionata e settore.`,
        "La stanza non era casuale: ogni enigma era generato, validato e risolvibile.",
        `Prossimo passo narrativo: NORA ha aperto un corridoio instabile verso una stanza di profondità ${nextDifficulty}.`,
      ],
      badges: run.mission.rewards.map((reward) => reward.label),
      createdAt: new Date().toISOString(),
    });

...
export class MainMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // Daily loop: advance the streak on a new day and auto-grant the completion
    // bonus if yesterday's/today's goals were finished before this open.
    ...
    // Daily loop chip: streak + today's session goals (opens the detail panel).
    const dailyObjectives = saveSystem.dailyObjectives();
    const dailyDone = dailyObjectives.filter((objective) => objective.done).length;
    new Button(this, 632, 112, `🔥 Serie ${saveSystem.dailyStreak}g · Missione del giorno ${dailyDone}/${dailyObjectives.length} ▸`,
      () => this.openDailyPanel(), { width: 300, height: 32, fontSize: 12, fill: 0x2a1f3a, stroke: 0xf6c85f });
    if (dailyGranted > 0) {
      this.time.delayedCall(400, () => this.showDailyRewardToast(dailyGranted));
    }
